...
class BasicBlock(M.Module):
    ...
    def __init__(self, inplanes, outplanes, stride=1, dilation=1, groups=1, downsample=None,
            base_width=64, norm_layer=None, se_module=None):
        '''
...
class ResNet(M.Module):
    def __init__(self,  block, blocks,in_ch=3, num_classes=1000, first_stride=2, light_head=False, zero_init_residual=False, 
            groups=1, width_per_group=64, strides=[1, 2, 2, 2], dilations=[1, 1, 1, 1],multi_grids=[1, 1, 1], norm_layer=None, se_module=None, reduction=16):
        '''
        ...
        if light_head:
            self.conv1 = M.Sequential(
                conv3x3(in_ch, self.inplanes, stride=first_stride),
        ...
        #4 stage
        self.layer1 = self._make_layer(block, 64, blocks[0], stride=strides[0], dilation=dilations[0], se_module=se_module, reduction=reduction)
        self.layer2 = self._make_layer(block, 128, blocks[1], stride=strides[1], dilation=dilations[1], se_module=se_module, reduction=reduction)
        self.layer3 = self._make_layer(block, 256, blocks[2], stride=strides[2], dilation=dilations[2], se_module=se_module, reduction=reduction)
        self.layer4 = self._make_grid_layer(block, 512, blocks[3], stride=strides[3], dilation=dilations[3], se_module=se_module, reduction=reduction)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, se_module=None, reduction=16):
        '''
        ...
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups, base_width=self.base_width,
                    dilation=dilation, norm_layer=norm_layer, se_module=se))
        return M.Sequential(*layers)
    
    def _make_grid_layer(self, block, planes, blocks, stride=1, dilation=1, se_module=None, reduction=16):
        '''
        ...
        layers.append(block(self.inplanes, planes, groups=self.groups, stride=stride, downsample=downsample,
                            base_width=self.base_width, dilation=dilation*self.multi_grids[0], norm_layer=norm_layer, se_module=se))
        self.inplanes = planes * block.expansion
        if se_module is not None:
            se = se_module(self.inplanes, reduction, norm_layer=self.norm_layer)
